<template>
  ...
  <section>
    <div class="container" v-for="job in computeddata" :key="job">
      <div class="numberLength">{{ job.score }}</div>
      <div class="main">
        <h5>
          <a :href="job.url" id="tag" target="_blank">{{ job.title }}</a> ({{
            job.url
          }})
        </h5>
        <p>{{ timeAgo(job.time) }} ago Job</p>
      </div>
    </div>
    <div class="hr"></div>
  </section>
  ...
  computed: {
    computeddata() {
      if (!this.jobNews) return [];
        ...
        return this.jobNews.slice(firstindex, lastindex);
      }
    },
    paginate_total() {
      const resminder = this.jobNews.length % this.per1page;
      if (resminder > 0) {
        return Math.floor(this.jobNews.length / this.per1page) + 1;
      } else {
        return this.jobNews.length / this.per1page;
      }
    },
  },
  async created() {
      ...
      .then((response) => {
        ...
        stories.forEach((storyId) => {
          axios
            ...
            .then((response) => {
              this.jobNews.push(response.data);
