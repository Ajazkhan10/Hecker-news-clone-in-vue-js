<template>
  ...
  <section>
    <div class="container" v-for="d in computeddata" :key="d">
      <div class="numberLength">{{ d.score }}</div>
      <div class="main">
        <!-- <ul>
            <li> -->
        <h5>
          <a :href="d.url" id="tag" target="_blank">{{ d.title }}</a>
        </h5>
        <!-- <span><a href="#">www.google.com</a></span> -->
        <p>
          by <router-link to="/singleAsk">{{ d.by }} | </router-link
          >{{ timeAgo(d.descendants) }} ago |
          <router-link to="/askComment"
            >{{ d.descendants }} comments</router-link
          >
        </p>
        <!-- </li>
          </ul> -->
  ...
  computed: {
    computeddata() {
      if (!this.askNews) return [];
        ...
        return this.askNews.slice(firstindex, lastindex);
      }
    },
    paginate_total() {
      const resminder = this.askNews.length % this.per1page;
      if (resminder > 0) {
        return Math.floor(this.askNews.length / this.per1page) + 1;
      } else {
        return this.askNews.length / this.per1page;
      }
    },
  },
  async created() {
      ...
      .then((response) => {
        ...
        stories.forEach((storyId) => {
          axios
            ...
            .then((response) => {
              this.askNews.push(response.data);
