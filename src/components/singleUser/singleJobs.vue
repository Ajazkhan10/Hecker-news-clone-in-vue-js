<template>
  <div class="section">
    ...
    <div class="links">
      <router-link :to="{ path: '/' + story.id }">Submissions</router-link> |
      <router-link :to="{ path: '/newsName/' + story.id }"
...
export default {
  ...
  async created() {
    await axios
      .get(
        `https://hacker-news.firebaseio.com/v0/user/${this.$route.params.name}.json`
      )
      .then((response) => {
